...
The newer versions of this tool also allow for taxon names to be found quickly, using the 'Name Search' link at the top right of the barcode hunter page. That takes you to a new page with a single search box and a dropdown for searching the SANBI checklist or WFO for names. To use the SANBI checklist, you need an extract from the SANBI taxon backbone (the SA National Plants Checklist) that has been converted to a SQLite file, inside the barcode hunter folder. This can be created with addtaxatodb.py or obtained from one of the NSCF staff members. To use WFO taxa a download of the WFO database will be required, and the SQLite file created with add_wfo_taxa_to_db.py, or  obtained from one of the NSCF staff members. 
